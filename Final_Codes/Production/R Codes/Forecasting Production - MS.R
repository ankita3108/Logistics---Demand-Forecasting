...
path4<-paste0(path,"/Production/Data Directory/Milho2 Input Files/")
path5<-paste0(path,"/Production/Data Directory/Jan2019/Milho2_Production_Model_Re-Run_Results/Milho2 Predicted Production New Results/")
path6<-paste0(path,"/Production/Data Directory/Jan2019/Milho2_Production_Model_Re-Run_Results/Milho2 Confidence New Results/")
...
agro_yearly$Start_int<-ifelse(agro_yearly$Month<=11,-1,0)
agro_yearly$Start_Year<-agro_yearly$Years+agro_yearly$Start_int

agro_yearly$End_int<-ifelse(agro_yearly$Month>11,1,0)
...
agro_yearly_pred<-subset(agro_yearly, UF=='MS' & Produto=='Milho 2 Safra' & Type=='Producao' & Start_Year>=(max_year-1))
...
agro_monthly_act<-subset(agro_yearly, UF=='MS' & Produto=='Milho 2 Safra' & Type=='Producao' & Start_Year>=(max_year-1))
...
## Yearly list
year_list<-subset(agro_yearly, UF=='MS' & Produto=='Milho 2 Safra' & Type=='Producao')
year_list<-year_list$Years
year_list

## Yearly values for 2012 to 2019
agro_yearly<-subset(agro_yearly, UF=='MS' & Produto=='Milho 2 Safra' & Type=='Producao')
...
set_input_dir()
data <- read.csv("Milho2_MS.csv", header = T, stringsAsFactors = FALSE)

data[is.na(data)] <- 0

data$Production<-data$Production+1

log_data<-log(data[,c(2:3)])
...
#ARIMAfit <- arima(ts_pred,order=c(0,0,0),seasonal=list(order=c(1,1,1),period=12),xreg = train_pred[,c(2)])
ARIMAfit <- auto.arima(ts_pred,xreg = train_pred[,c(2)])
...
set_plot_dir()
jpeg('Milho2_MS.jpg')
plot(org_ts,ylab="Production ('000 tonnes)", main="Production Forecast - Milho2 (MS)")
lines(pred,col="red")
dev.off()
...
set_predicted_dir()
write.csv(dataframe,file = "Predicted-Production_MS.csv",row.names=FALSE)
...
data_new$Start_int<-ifelse(data_new$Month<=11,-1,0)
data_new$Start_Year<-data_new$Year+data_new$Start_int

data_new$End_int<-ifelse(data_new$Month>11,1,0)
...
write.csv(new_dataframe,file = "Predicted-Production_MS.csv",row.names=FALSE)
...
set_confidence_dir()
write.csv(confidence,file = "Confidence_MS.csv",row.names=FALSE)
...
df2$Start_int<-ifelse(df2$Month<=11,-1,0)
df2$Start_Year<-df2$Year+df2$Start_int

df2$End_int<-ifelse(df2$Month>11,1,0)
...
write.csv(confidence_new_data,file = "Confidence_MS.csv",row.names=FALSE)
set_plot_dir()