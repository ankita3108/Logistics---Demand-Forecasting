...
path2<-paste0(path,"/Production/Data Directory/Milho1 Plantation Input Files/")
path3<-paste0(path,"/Production/Data Directory/Milho1 Plantation Output/")
...
set_input_dir()
data <- read.csv("Milho1_RS.csv", header = T, stringsAsFactors = FALSE)
str(data)

data$Plantation<-data$Plantation+1
log_data<-log(data[,c(2:6)])
...
pred_test <- forecast(ARIMAfit,h=max_len,xreg = test_pred[,c(2:5)])
...
set_plot_dir()
jpeg('Milho1_RS.jpg')
plot(org_ts,ylab="Plantation ('000 ha)", main="Plantation Forecast - Milho1 (RS)")
lines(pred,col="red")
dev.off()
...
set_output_dir()
write.csv(dataframe,file = "Predicted-Plantation_RS.csv",row.names=FALSE)
