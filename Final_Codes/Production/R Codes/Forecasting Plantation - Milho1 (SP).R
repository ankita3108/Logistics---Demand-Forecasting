...
set_input_dir()
data <- read.csv("Milho1_SP.csv", header = T, stringsAsFactors = FALSE)
str(data)
...
## Manual Arima Fit
#ARIMAfit <- arima(ts_pred,order=c(0,0,0),seasonal=list(order=c(1,1,1),period=12),xreg = train_pred[,c(2:4)])
...
set_plot_dir()
jpeg('Milho1_SP.jpg')
plot(org_ts,ylab="Plantation ('000 ha)", main="Plantation Forecast - Milho1 (SP)")
lines(pred,col="red")
dev.off()
...
set_output_dir()
write.csv(dataframe,file = "Predicted-Plantation_SP.csv",row.names=FALSE)