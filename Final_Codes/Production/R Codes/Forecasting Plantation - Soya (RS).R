...
set_input_dir()
data <- read.csv("Soya_RS.csv", header = T, stringsAsFactors = FALSE)
str(data)
...
## Manual Arima Fit
#ARIMAfit <- arima(ts_pred,order=c(0,0,0),seasonal=list(order=c(2,1,0),period=12),xreg = train_pred[,c(2:5)])
...
set_plot_dir()
jpeg('Soya_RS.jpg')
plot(org_ts,ylab="Plantation ('000 ha)", main="Plantation Forecast - Soya (RS)")
lines(pred,col="red")
dev.off()
