...
set_input_dir()
data <- read.csv("Soya_MS.csv", header = T, stringsAsFactors = FALSE)
str(data)
...
pred_test <- forecast(ARIMAfit,h=12,xreg = test_pred[,c(2:4)])
...
set_plot_dir()
jpeg('Soya_MS.jpg')
plot(org_ts,ylab="Plantation ('000 ha)", main="Plantation Forecast - Soya (MS)")
lines(pred,col="red")
dev.off()
...
set_output_dir()
write.csv(dataframe,file = "Predicted-Plantation_MS.csv",row.names=FALSE)