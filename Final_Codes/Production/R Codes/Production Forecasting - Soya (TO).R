...
path4<-paste0(path,"/Production/Data Directory/Soya Input Files/")
path5<-paste0(path,"/Production/Data Directory/Jan2019/Production_Re-Run_Results_-_Soya/Soya_Predicted_Production_New_Results/")
path6<-paste0(path,"/Production/Data Directory/Jan2019/Production_Re-Run_Results_-_Soya/Soya_Confidence_New_Results/")
...
agro_yearly_pred<-subset(agro_yearly, UF=='TO' & Produto=='Soja' & Type=='Producao' & Start_Year>=(max_year-1))
...
agro_monthly_act<-subset(agro_yearly, UF=='TO' & Produto=='Soja' & Type=='Producao' & Start_Year>=(max_year-1))
...
## Yearly list
year_list<-subset(agro_yearly, UF=='TO' & Produto=='Soja' & Type=='Producao')
year_list<-year_list$Years
year_list

## Yearly values for 2012 to 2019
agro_yearly<-subset(agro_yearly, UF=='TO' & Produto=='Soja' & Type=='Producao')
...
set_input_dir()
routes <- read.csv("Soya_TO.csv", header = T, stringsAsFactors = FALSE)
routes[is.na(routes)] <- 0

routes$Production<-routes$Production+1
routes$Lag4_Plantation<-routes$Lag4_Plantation+1
routes$Lag3_Plantation<-routes$Lag3_Plantation+1
routes$Lag2_Plantation<-routes$Lag2_Plantation+1
routes$Plantation<-routes$Plantation+1
routes$Exchange_Rate<-NULL
routes$Interest_Rate<-NULL
routes$Milho_Price<-NULL
routes$Lag3_Milho_Price<-NULL

log_data<-log(routes[,c(2:6)])
...
## Manual Arima Fit
#ARIMAfit <- arima(ts_pred,order=c(0,0,0),seasonal=list(order=c(1,1,0),period=12),xreg = train_pred[,c(2:5)])
ARIMAfit <- auto.arima(ts_pred,xreg = train_pred[,c(2:5)])

pred_test <- forecast(ARIMAfit,h=max_len,xreg = test_pred[,c(2:5)],level = c(70,75,80,85,90,95))
...
set_plot_dir()
jpeg('Soya_TO.jpg')
plot(org_ts,ylab="Production ('000 tonnes)", main="Production Forecast - Soya (TO)")
lines(pred,col="red")
dev.off()
...
set_predicted_dir()
write.csv(dataframe,file="Predicted-Production_TO.csv",row.names = FALSE)
...
write.csv(new_dataframe,file="Predicted-Production_TO.csv",row.names = FALSE)
...
set_confidence_dir()
write.csv(confidence,file="Confidence_TO.csv",row.names = FALSE)
...
write.csv(confidence_new_data,file="Confidence_TO.csv",row.names = FALSE)
set_plot_dir()