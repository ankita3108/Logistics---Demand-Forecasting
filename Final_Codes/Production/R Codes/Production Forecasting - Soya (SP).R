...
agro_yearly_pred<-subset(agro_yearly, UF=='SP' & Produto=='Soja' & Type=='Producao' & Start_Year>=(max_year-1))
...
agro_monthly_act<-subset(agro_yearly, UF=='SP' & Produto=='Soja' & Type=='Producao' & Start_Year>=(max_year-1))
...
## Yearly list
year_list<-subset(agro_yearly, UF=='SP' & Produto=='Soja' & Type=='Producao')
year_list<-year_list$Years
year_list

## Yearly values for 2012 to 2019
agro_yearly<-subset(agro_yearly, UF=='SP' & Produto=='Soja' & Type=='Producao')
...
set_input_dir()
routes <- read.csv("Soya_SP.csv", header = T, stringsAsFactors = FALSE)
...
set_plot_dir()
jpeg('Soya_SP.jpg')
plot(org_ts,ylab="Production ('000 tonnes)", main="Production Forecast - Soya (SP)")
lines(pred,col="red")
dev.off()
