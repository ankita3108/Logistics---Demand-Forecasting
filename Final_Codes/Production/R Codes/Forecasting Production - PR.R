...
agro_yearly_pred<-subset(agro_yearly, UF=='PR' & Produto=='Milho 2 Safra' & Type=='Producao' & Start_Year>=(max_year-1))
...
agro_monthly_act<-subset(agro_yearly, UF=='PR' & Produto=='Milho 2 Safra' & Type=='Producao' & Start_Year>=(max_year-1))
...
## Yearly list
year_list<-subset(agro_yearly, UF=='PR' & Produto=='Milho 2 Safra' & Type=='Producao')
year_list<-year_list$Years
year_list

## Yearly values for 2012 to 2019
agro_yearly<-subset(agro_yearly, UF=='PR' & Produto=='Milho 2 Safra' & Type=='Producao')
...
set_input_dir()
data <- read.csv("Milho2_PR.csv", header = T, stringsAsFactors = FALSE)
...
#ARIMAfit <- arima(ts_pred,order=c(0,0,0),seasonal=list(order=c(1,1,0),period=12),xreg = train_pred[,c(2)])
...
set_plot_dir()
jpeg('Milho2_PR.jpg')
plot(org_ts,ylab="Production ('000 tonnes)", main="Production Forecast - Milho2 (PR)")
lines(pred,col="red")
dev.off()
...
set_predicted_dir()
write.csv(dataframe,file = "Predicted-Production_PR.csv",row.names=FALSE)
...
write.csv(new_dataframe,file = "Predicted-Production_PR.csv",row.names=FALSE)
...
set_confidence_dir()
write.csv(confidence,file = "Confidence_PR.csv",row.names=FALSE)
...
write.csv(confidence_new_data,file = "Confidence_PR.csv",row.names=FALSE)
set_plot_dir()
