...
set_input_dir()
data <- read.csv("Milho1_MA.csv", header = T, stringsAsFactors = FALSE)
str(data)
...
## Manual Arima Fit
#ARIMAfit <- arima(ts_pred,order=c(0,0,0),seasonal=list(order=c(0,1,0),period=12),xreg = train_pred[,c(2:4)])
...
set_plot_dir()
jpeg('Milho1_MA.jpg')
plot(org_ts,ylab="Plantation ('000 ha)", main="Plantation Forecast - Milho1 (MA)")
lines(pred,col="red")
dev.off()
...
set_output_dir()
write.csv(dataframe,file = "Predicted-Plantation_MA.csv",row.names=FALSE)
