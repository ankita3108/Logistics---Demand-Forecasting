...
agro_yearly_pred<-subset(agro_yearly, UF=='GO' & Produto=='Milho 1 Safra' & Type=='Producao' & Start_Year>=(max_year-1))
...
agro_monthly_act<-subset(agro_yearly, UF=='GO' & Produto=='Milho 1 Safra' & Type=='Producao' & Start_Year>=(max_year-1))
...
## Yearly list
year_list<-subset(agro_yearly, UF=='GO' & Produto=='Milho 1 Safra' & Type=='Producao')
year_list<-year_list$Years
year_list

## Yearly values for 2012 to 2019
agro_yearly<-subset(agro_yearly, UF=='GO' & Produto=='Milho 1 Safra' & Type=='Producao')
...
set_input_dir()
routes <- read.csv("Milho1_GO.csv", header = T, stringsAsFactors = FALSE)
...
set_plot_dir()
jpeg('Milho1_GO.jpg')
plot(org_ts,ylab="Production ('000 tonnes)", main="Production Forecast - Milho1 (GO)")
lines(pred,col="red")
dev.off()
