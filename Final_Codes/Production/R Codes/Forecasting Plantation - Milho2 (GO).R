...
path2<-paste0(path,"/Production/Data Directory/Milho2 Plantation Input Files/")
path3<-paste0(path,"/Production/Data Directory/Milho2 Plantation Output/")
...
## Manual Arima Fit
#ARIMAfit <- arima(ts_pred,order=c(0,0,0),seasonal=list(order=c(2,1,0),period=12),xreg = train_pred[,c(2:4)])
...
set_plot_dir()
jpeg('Milho2_GO.jpg')
plot(org_ts,ylab="Plantation ('000 ha)", main="Plantation Forecast - Milho2 (GO)")
lines(pred,col="red")
dev.off()
...
set_output_dir()
write.csv(dataframe,file = "Predicted-Plantation_GO.csv",row.names=FALSE)
