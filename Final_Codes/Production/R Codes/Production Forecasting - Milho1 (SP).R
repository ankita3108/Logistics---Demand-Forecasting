...
agro_yearly_pred<-subset(agro_yearly, UF=='SP' & Produto=='Milho 1 Safra' & Type=='Producao' & Start_Year>=(max_year-1))
...
agro_monthly_act<-subset(agro_yearly, UF=='SP' & Produto=='Milho 1 Safra' & Type=='Producao' & Start_Year>=(max_year-1))
...
## Yearly list
year_list<-subset(agro_yearly, UF=='SP' & Produto=='Milho 1 Safra' & Type=='Producao')
year_list<-year_list$Years
year_list

## Yearly values for 2012 to 2019
agro_yearly<-subset(agro_yearly, UF=='SP' & Produto=='Milho 1 Safra' & Type=='Producao')
...
set_input_dir()
routes <- read.csv("Milho1_SP.csv", header = T, stringsAsFactors = FALSE)
...
routes$Soya_Price<-NULL
routes$Lag1_Soya_Price<-NULL
routes$Lag3_Soya_Price<-NULL
...
set_plot_dir()
jpeg('Milho1_SP.jpg')
plot(org_ts,ylab="Production ('000 tonnes)", main="Production Forecast - Milho1 (SP)")
lines(pred,col="red")
dev.off()
...
set_predicted_dir()
write.csv(dataframe,file="Predicted-Production_SP.csv",row.names = FALSE)
...
write.csv(new_dataframe,file="Predicted-Production_SP.csv",row.names = FALSE)
...
set_confidence_dir()
write.csv(confidence,file="Confidence_SP.csv",row.names = FALSE)
...
write.csv(confidence_new_data,file="Confidence_SP.csv",row.names = FALSE)
set_plot_dir()