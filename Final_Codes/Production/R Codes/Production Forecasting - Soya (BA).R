...
agro_yearly_pred<-subset(agro_yearly, UF=='BA' & Produto=='Soja' & Type=='Producao' & Start_Year>=(max_year-1))
...
agro_monthly_act<-subset(agro_yearly, UF=='BA' & Produto=='Soja' & Type=='Producao' & Start_Year>=(max_year-1))
...
## Yearly list
year_list<-subset(agro_yearly, UF=='BA' & Produto=='Soja' & Type=='Producao')
year_list<-year_list$Years
year_list

## Yearly values for 2012 to 2019
agro_yearly<-subset(agro_yearly, UF=='BA' & Produto=='Soja' & Type=='Producao')
...
set_input_dir()
routes <- read.csv("Soya_BA.csv", header = T, stringsAsFactors = FALSE)
...
routes$Lag1_Milho_Price<-NULL
routes$Lag3_Milho_Price<-NULL

log_data<-log(routes[,c(2:5)])
sum(is.na(log_data))
...
pred_test <- forecast(ARIMAfit,h=max_len,xreg = test_pred[,c(2:4)],level = c(70,75,80,85,90,95))
...
set_plot_dir()
jpeg('Soya_BA.jpg')
plot(org_ts,ylab="Production ('000 tonnes)", main="Production Forecast - Soya (BA)")
lines(pred,col="red")
dev.off()
