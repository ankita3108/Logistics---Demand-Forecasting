...
agro_yearly_pred<-subset(agro_yearly, UF=='PI' & Produto=='Soja' & Type=='Producao' & Start_Year>=(max_year-1))
...
agro_monthly_act<-subset(agro_yearly, UF=='PI' & Produto=='Soja' & Type=='Producao' & Start_Year>=(max_year-1))
...
## Yearly list
year_list<-subset(agro_yearly, UF=='PI' & Produto=='Soja' & Type=='Producao')
year_list<-year_list$Years
year_list

## Yearly values for 2012 to 2019
agro_yearly<-subset(agro_yearly, UF=='PI' & Produto=='Soja' & Type=='Producao')
...
set_input_dir()
routes <- read.csv("Soya_PI.csv", header = T, stringsAsFactors = FALSE)
...
set_plot_dir()
jpeg('Soya_PI.jpg')
plot(org_ts,ylab="Production ('000 tonnes)", main="Production Forecast - Soya (PI)")
lines(pred,col="red")
dev.off()
...
set_predicted_dir()
write.csv(dataframe,file="Predicted-Production_PI.csv",row.names = FALSE)
...
write.csv(new_dataframe,file="Predicted-Production_PI.csv",row.names = FALSE)
...
set_confidence_dir()
write.csv(confidence,file="Confidence_PI.csv",row.names = FALSE)
...
write.csv(confidence_new_data,file="Confidence_PI.csv",row.names = FALSE)
set_plot_dir()