...
agro_yearly_pred<-subset(agro_yearly, UF=='MA' & Produto=='Milho 1 Safra' & Type=='Producao' & Start_Year>=(max_year-1))
...
agro_monthly_act<-subset(agro_yearly, UF=='MA' & Produto=='Milho 1 Safra' & Type=='Producao' & Start_Year>=(max_year-1))
...
## Yearly list
year_list<-subset(agro_yearly, UF=='MA' & Produto=='Milho 1 Safra' & Type=='Producao')
year_list<-year_list$Years
year_list

## Yearly values for 2012 to 2019
agro_yearly<-subset(agro_yearly, UF=='MA' & Produto=='Milho 1 Safra' & Type=='Producao')
...
set_input_dir()
routes <- read.csv("Milho1_MA.csv", header = T, stringsAsFactors = FALSE)
...
#ARIMAfit <- arima(ts_pred,order=c(0,0,0),seasonal=list(order=c(2,1,0),period=12),xreg = train_pred[,c(2:3)])
...
set_plot_dir()
jpeg('Milho1_MA.jpg')
plot(org_ts,ylab="Production ('000 tonnes)", main="Production Forecast - Milho1 (MA)")
lines(pred,col="red")
dev.off()
